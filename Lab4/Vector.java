...
public class Vector {
    ...
    // NEW METHOD FROM LAB 3: 

    public Vector augment(){
        int newDim = this.getDimension() + 1; 
        double[] numbers = this.getNumbers();
        double[] augmentedArray = new double[newDim]; 
        int i = 0; 
        for(double num : numbers){
            augmentedArray[i] = num; 
            i++; 
        }
        augmentedArray[newDim - 1] = 1; 
        return new Vector(augmentedArray); 
    }
}